...
func _integrate_forces(state: PhysicsDirectBodyState2D):
	...
	var input_left = "Left1" if player_id == 1 else "Left1"
	var input_right = "Right1" if player_id == 1 else "Right1"
	var input_jump = "Jump1" if player_id == 1 else "Jump1"
	...
	if Input.is_action_just_pressed(input_jump):
		...
		animation_player.play("1Jump")
	
	
	for i in range(state.get_contact_count()):
		var collider = state.get_contact_collider_object(i)
		if collider is Panda2 and collider != self:
